...
const RecentOrdersTable = ({ orders, handleVerifyPayment }: RecentOrdersTableProps) => {
    return (
        <div className="bg-secondary rounded-[2.5rem] border border-theme overflow-hidden shadow-xl">
            <div className="p-8 border-b border-theme flex justify-between items-center">
                <h3 className="text-2xl font-serif font-black text-primary-text tracking-widest uppercase">
                    Live <span className="text-brand italic font-normal">Directives</span>
                </h3>
                <div className="relative w-64">
                    <Search size={16} className="absolute left-4 top-1/2 -translate-y-1/2 text-brand opacity-60" />
                    <input
                        type="text"
                        placeholder="Scan Order Registry..."
                        className="w-full pl-12 pr-6 py-3 bg-bg-alt border border-theme rounded-2xl text-[10px] font-black uppercase tracking-widest text-primary-text focus:outline-none focus:border-brand/40 transition-all placeholder:text-secondary-text/50"
                    />
                </div>
            </div>
                    ...
                    <tbody className="divide-y divide-theme text-sm">
                        {(Array.isArray(orders) ? orders : []).length > 0 ? (
                            (Array.isArray(orders) ? orders : []).map((order: any) => (
                                <tr key={order._id} className="hover:bg-bg-alt transition-colors group text-primary-text">
                                    ...
                                    <td className="px-6 py-4">
                                        {order.paymentResult?.status === 'Pending Verification' ? (
                                            <div className="flex flex-col gap-1">
                                                ...
                                                <span className="text-[10px] text-secondary-text font-mono italic">UTR: {order.paymentResult.id}</span>
                                            </div>
                                        ) : (
